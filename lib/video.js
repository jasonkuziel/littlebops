...
export async function startVideoGeneration(options) {
  var { childName, story, orderId, webhookUrl } = options;

  var scene = generateScenePrompt(childName, story);

  // For official models on Replicate, use the /models/ endpoint
  // instead of /predictions with a version hash
  var response = await fetch("https://api.replicate.com/v1/models/minimax/video-01/predictions", {
    method: "POST",
    headers: {
...
function generateScenePrompt(childName, story) {
  var baseScene = "A cute colorful animated children's music video scene. " +
