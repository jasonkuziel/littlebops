...
export async function generateLyrics(childName, age, story, genre, mood) {
  ...
  const genreDesc = GENRE_DESCRIPTIONS[genre] || GENRE_DESCRIPTIONS.pop;
  const moodDesc = MOOD_DESCRIPTIONS[mood] || MOOD_DESCRIPTIONS.energetic;

  const prompt = "You are a professional children's songwriter. Write a fun, age-appropriate song for a child.\n\n" +
...
export function generateTemplateLyrics(childName, age, story) {
  const ageText = age ? (age + " years old") : "so amazing";
  return [
    "[Verse 1]",
...
export async function submitSunoGeneration(lyrics, childName, genre, mood, callbackUrl) {
  let style = GENRE_STYLES[genre] || GENRE_STYLES.pop;
  const moodDesc = MOOD_DESCRIPTIONS[mood] || MOOD_DESCRIPTIONS.energetic;
  style = style + ", " + moodDesc;

  let title = childName + "'s Song";
  if (title.length > 80) title = title.substring(0, 80);

  console.log("Submitting Suno generation...");
  console.log("Style: " + style);
  console.log("Callback: " + callbackUrl.replace(/token=[^&]+/, "token=[REDACTED]"));
