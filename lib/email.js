...
export async function sendSongReadyEmail(options) {
  const { to, childName, songUrl, lyrics, successPageUrl } = options;

  // Ensure name is properly capitalized
  const name = capitalizeWords(childName || "");

  const html = buildEmailHtml(name, songUrl, lyrics, successPageUrl);
...
export function buildEmailHtml(childName, songUrl, lyrics, successPageUrl) {
  const safeName = escapeHtml(childName);
  const safeSongUrl = escapeHtml(songUrl);
  const safeSuccessUrl = escapeHtml(successPageUrl);
  ...
  const lyricsHtml = lyrics
    ...
    .map(function(line) {
      ...
      if (line.startsWith("[")) {
        return '<p style="color: #FF6B4A; font-weight: 700; font-size: 13px; margin: 16px 0 4px 0; text-transform: uppercase; letter-spacing: 1px;">' + safeLine + '</p>';
      }
      if (line.trim() === "") return '<br/>';
      return '<p style="color: #1B1340; margin: 2px 0; font-size: 15px; line-height: 1.7;">' + safeLine + '</p>';
    })
    .join("\n");

  const appUrl = escapeHtml(process.env.NEXT_PUBLIC_APP_URL || "https://getlittlebops.com");
...
\
  <div style="text-align: center; margin-bottom: 36px;">\
    ...
    <h1 style="font-size: 26px; color: #1B1340; margin: 0 0 8px 0; font-weight: 800;">' + safeName + '\'s song is ready!</h1>\
    <p style="color: #4A4270; font-size: 16px; margin: 0;">A one-of-a-kind song, made with love.</p>\
  </div>\
\
  <div style="text-align: center; margin-bottom: 20px;">\
    <a href="' + safeSuccessUrl + '" style="display: inline-block; background: #FF6B4A; color: #fff; text-decoration: none; padding: 16px 36px; border-radius: 14px; font-size: 17px; font-weight: 700;">Listen to ' + safeName + '\'s Song →</a>\
  </div>\
\
  <div style="text-align: center; margin-bottom: 32px;">\
    <a href="' + safeSongUrl + '" style="display: inline-block; color: #FF6B4A; text-decoration: none; padding: 12px 28px; border-radius: 14px; font-size: 15px; font-weight: 700; border: 2px solid #FF6B4A;">↓ Download MP3</a>\
  </div>\
\
